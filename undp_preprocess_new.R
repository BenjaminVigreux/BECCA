...
for (ncountry in 1:Ncountry) {
  ...
  ind <- grep("feel",names(data[[ncountry]]), ignore.case = TRUE)
  ...
  levels(data[[ncountry]]$Feeling)[levels(data[[ncountry]]$Feeling)=="Very negative - Veoma negativna"] <- "Strongly negative"
  levels(data[[ncountry]]$Feeling)[levels(data[[ncountry]]$Feeling)=="Negative - Negativna"] <- "Negative"
  levels(data[[ncountry]]$Feeling)[levels(data[[ncountry]]$Feeling)=="negative"] <- "Negative"
  levels(data[[ncountry]]$Feeling)[levels(data[[ncountry]]$Feeling)=="Neutral - Neutralna"] <- "Neutral"
  levels(data[[ncountry]]$Feeling)[levels(data[[ncountry]]$Feeling)=="neutral"] <- "Neutral"
  levels(data[[ncountry]]$Feeling)[levels(data[[ncountry]]$Feeling)=="Positive - Pozitivna"] <- "Positive"
  ...
  levels(data[[ncountry]]$Feeling)[levels(data[[ncountry]]$Feeling)=="Very positive - Veoma pozitivna"] <- "Strongly positive"
  ...
  data[[ncountry]]$timedate <- as.character(dum)
  # Convert to day-time format
  # from p. 31 of the xlsx pdf guide https://cran.r-project.org/web/packages/xlsx/xlsx.pdf
  data[[ncountry]]$timedate_num <- as.numeric(as.POSIXct(data[[ncountry]]$timedate,format="%m/%d/%Y  %H:%M:%S",tz="GMT", origin="1970-01-01"))
  #data[[ncountry]]$timedate_num <- as.POSIXct((data[[ncountry]]$timedate_num-25569)*86400, tz="GMT", origin="1970-01-01")
