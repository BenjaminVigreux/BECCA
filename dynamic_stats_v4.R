...
##  Integrate UI with flexdashboard.
##  Make Pretty!

  ##  ISSUES

##  Cannot deselect checkboxes, which crashes app when dataset is changed.
##  Cannot deselect tier 2 choices. May have to move all renderUI(switch()) content to UI and make conditional with updateSelectInput.
...
ui <- shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(fluidRow(tabsetPanel(type = "tabs",
      tabPanel("Rows",p(textOutput("x_sel")),tableOutput("x_sum"),uiOutput("x_chosen"),class = "leftAlign"),
      tabPanel("Columns",p(textOutput("y_sel")),tableOutput("y_sum"),uiOutput("y_chosen"),class = "leftAlign"))),
      fluidRow(tabsetPanel(
      tabPanel("Number of Stories",uiOutput("stats")),
      tabPanel("Percentages",uiOutput("props")))))
  )
))
...
##################################################  SERVER  ##################################################
  ...
  server <- shinyServer(function(input,output,session){
    ...
    output$x_sum <- renderTable({
      req(length(c(input$x_t1,input$x_d1,input$x_s1)) == 1)
    ...
    output$y_sum <- renderTable({
      req(length(c(input$y_t1,input$y_d1,input$y_s1)) == 1)
    ...
    observe({
       req(input$x_t1)
      if (input$x1 != "Triads") {sel <- "Select"} else {sel <- input$x_t1}
      browser()
        updateSelectInput(session, "x_t1", choices = c("Select",names(triads)), selected = sel)
    })
    observe({
      req(input$x_d1)
        updateSelectInput(session, "x_d1", choices = c("Select",names(dyads)), selected = if (input$x1 != "Dyads") "Select" else input$x_d1)
    })
    observe({
      req(input$x_s1)
        updateSelectInput(session, "x_s1", choices = c("Select",names(stones)), selected = if (input$x1 != "Stones") "Select" else input$x_s1)
    })
    observe({
      req(input$y_t1)
        updateSelectInput(session, "y_t1", choices = c("Select",names(triads)), selected = if (input$y1 != "Triads") "Select" else input$y_t1)
      })
    observe({
      req(input$y_d1)
        updateSelectInput(session, "y_d1", choices = c("Select",names(dyads)), selected = if (input$y1 != "Dyads") "Select" else input$x_d1)
    })
    observe({
      req(input$y_s1)
        updateSelectInput(session, "y_s1", choices = c("Select",names(stones)), selected = if (input$y1 != "Stones") "Select"  else input$x_s1)
    })
