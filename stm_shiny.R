...
stm.progress <- function(documents, vocab, settings, model=NULL) {
  ...
  while(!stopits) {
    ...
    #Convergence
    source("stm_shiny_conv.R")
    convergence <- charlie.conv(bound.ss, convergence, settings)
    ...
    if(!stopits & verbose) {report(convergence, ntokens=ntokens, beta, vocab, 
      ...
      dist <- rel.chan
    setProgress(message = paste("Distance from convergence...",dist),
                 value = abs(log(abs(rel.chan))),
                 min = 0,
                 max = abs(log(emtol)))}
    #settings$convergence$em.converge.thresh
  }
  #######
